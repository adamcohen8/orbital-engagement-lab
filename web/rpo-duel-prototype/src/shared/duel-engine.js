...
export function createDuelRound({ pairSeed, rules = DUEL_PROTOTYPE_RULES } = {}) {
  ...
  return {
    ...
    setControls(role, nextControls, { playerId = null, sequence = null } = {}) {
      ...
      if (inputEvents.length >= cfg.max_input_events) {
        throw new Error("Round input-event limit reached; further control changes are refused.");
      }
...
function normalizeRules(value) {
  ...
  for (const name of [
    ...
    "history_stride_ticks", "max_history_samples", "max_input_events",
