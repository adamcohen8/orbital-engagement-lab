...
export function createPrototypeServer({ staticRoot = STATIC_ROOT, roomRules = DUEL_PROTOTYPE_RULES } = {}) {
  ...
  const timer = setInterval(() => {
    ...
    for (const [code, room] of rooms) {
      room.tick(now);
      const staleWaiting = room.phase === "waiting" && now - room.updatedAt > 30 * 60 * 1000;
      const staleComplete = room.phase === "complete" && now - room.updatedAt > 10 * 60 * 1000;
      if (staleWaiting || staleComplete) rooms.delete(code);
    }
  }, TICK_INTERVAL_MS);
  timer.unref();
