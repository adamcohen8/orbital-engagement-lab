...
| Use a TLE | TLE initializes ECI state, then OEL numerically propagates configured dynamics | `examples/configs/public_tle_2hr_propagation.yaml`, `examples/configs/public_ground_station_access_from_tle.yaml` | Initial-state notes, force-model config, `object_state`, plots or CSV when enabled | User expects SGP4, current catalog freshness, or realistic force modeling | SGP4/general-perturbations propagation |
