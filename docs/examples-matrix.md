...
| `public_attitude_hold_disturbance.yaml` | Five-minute attitude hold under initial error and gravity-gradient disturbance torque | attitude histories, control artifacts, and review store | Headless-safe and useful for attitude-control sanity checks. |
...
| `public_mission_recovery_planner.yaml` | Mission-recovery planner recommendations after a small in-track perturbing burn and ten-minute coast | recovery trade-space plot, summary JSON, and review store | Demonstrates the public mission recovery/reconstitution workflow without campaign machinery. |
