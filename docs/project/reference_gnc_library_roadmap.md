...
This split should stay visible in docs, YAML examples, supported catalogs, benchmark
...
7. A controller-bench case when performance comparison or tuning is part of
   the user story.
8. supported catalog, CLI, and config exposure when non-developer users should select it.
...
This assessment is based on source inspection, docs/config usage, supported catalog exposure,
...
1. Mark compatibility/internal surfaces in docs and supported catalogs.
...
6. Add supported catalog exposure only after the docs and config exist.
