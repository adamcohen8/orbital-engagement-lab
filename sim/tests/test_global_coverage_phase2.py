...
def test_wgs84_cell_center_round_trip_and_periodic_longitude() -> None:
    ...
    np.testing.assert_array_equal(antimeridian, [6912, 6912])
...
def test_high_order_nested_identity_matches_frozen_astropy_healpix_vectors() -> None:
    # Generated independently with Astropy HEALPix rather than either OEL
    # forward/inverse implementation.
    reference = {
        5: {
            "pixels": [0, 1023, 4096, 10581, 12287],
            "lon": [0.7853981633974483, 0.7853981633974482, 0.0, 4.6878452877785195, 5.497787143782138],
            "lat": [0.020834840668648383, 1.5452801164374776, -0.7021142756051658, -0.7297276562269663, -0.020834840668648383],
            "mapped": [6912, 2816, 3754, 4864, 8447, 6741, 1023, 8192],
        },
        6: {
            "pixels": [0, 4095, 16384, 42325, 49151],
            "lon": [0.7853981633974483, 0.7853981633974482, 0.0, 4.700117134081604, 5.497787143782138],
            "lat": [0.01041685505597739, 1.5580384811972257, -0.7158380602251112, -0.7297276562269663, -0.01041685505597739],
            "mapped": [27648, 11264, 15018, 19456, 33791, 26965, 4095, 32768],
        },
        7: {
            "pixels": [0, 16383, 65536, 169301, 196607],
            "lon": [0.7853981633974482, 0.7853981633974483, 0.0, 4.706253057233147, 5.497787143782138],
            "lat": [0.005208356881134768, 1.564417436441463, -0.7227615888992984, -0.7297276562269663, -0.005208356881134768],
            "mapped": [110592, 45056, 60074, 77824, 135167, 107861, 16383, 131072],
        },
        8: {
            "pixels": [0, 65535, 262144, 677205, 786431],
            "lon": [0.7853981633974483, 0.7853981633974483, 0.0, 4.709321018808918, 5.497787143782138],
            "lat": [0.0026041696101148973, 1.5676068856737853, -0.726239235110326, -0.7297276562269663, -0.0026041696101148973],
            "mapped": [442368, 180224, 240298, 311296, 540671, 431445, 65535, 524288],
        },
    }
    point_lon_deg = np.array([-180.0, -135.0, -90.0, 0.0, 45.0, 179.999, 0.0, 45.0])
    point_authalic_lat_deg = np.array([0.0, 41.8103149, 41.8103149, 0.0, -41.8103149, 0.0, 89.9, -89.9])
    for order, values in reference.items():
        pixels = np.asarray(values["pixels"], dtype=np.int64)
        latitude, longitude = healpix_nested_centers_authalic(order, pixels)
        # The independently generated Astropy values and OEL's closed-form
        # centers can differ by a few binary64 ulps near the poles.
        np.testing.assert_allclose(latitude, values["lat"], rtol=0.0, atol=2.0e-14)
        np.testing.assert_allclose(
            np.mod(longitude, 2.0 * np.pi), values["lon"], rtol=0.0, atol=2.0e-14
        )
        mapped = healpix_nested_cell_indices(
            order, np.deg2rad(point_authalic_lat_deg), np.deg2rad(point_lon_deg)
        )
        np.testing.assert_array_equal(mapped, values["mapped"])


@pytest.mark.parametrize(
    ("order", "expected_count"),
    ((5, 12), (6, 38), (7, 156), (8, 620)),
)
def test_global_coverage_matches_frozen_astropy_healpix_geometry_counts(
    order: int,
    expected_count: int,
) -> None:
    """End-to-end oracle: 500 km equatorial nadir cone, 35 degree half angle.

    Counts were produced with Astropy HEALPix 1.0.3 plus an independent WGS84
    authalic-latitude inversion and convex-surface visibility calculation.
    """

    context = FrameContext(jd_utc_start=2451545.0)
    times = np.array([0.0, 1.0])
    observer_ecef = np.array([WGS84_A_KM + 500.0, 0.0, 0.0])
    boresight_ecef = np.array([-1.0, 0.0, 0.0])
    positions: list[np.ndarray] = []
    attitudes: list[np.ndarray] = []
    for time_s in times:
        ecef_from_eci = eci_to_ecef_rotation_context(float(time_s), context)
        positions.append(ecef_from_eci.T @ observer_ecef)
        boresight_eci = ecef_from_eci.T @ boresight_ecef
        body_y_eci = np.array([0.0, 0.0, 1.0])
        body_x_eci = np.cross(body_y_eci, boresight_eci)
        body_x_eci /= np.linalg.norm(body_x_eci)
        attitudes.append(
            dcm_to_quaternion_bn(
                np.vstack((body_x_eci, body_y_eci, boresight_eci))
            )
        )
    result = evaluate_global_coverage(
        GlobalCoverageConfig(
            analysis_id=f"astropy_reference_order_{order}",
            source_asset_id="spacecraft",
            state_provider_id="frozen_reference",
            attitude_source_kind="analytic_ideal",
            attitude_provider_id="frozen_reference_nadir",
            sensor_id="cone_35deg",
            order=order,
            half_angle_rad=np.deg2rad(35.0),
            quat_body_from_sensor=(1.0, 0.0, 0.0, 0.0),
        ),
        times_s=times,
    ...
    np.testing.assert_array_equal(result.covered_cell_count, expected_count)


@pytest.mark.parametrize("order", [True, 5.5])
def test_all_direct_healpix_mapping_helpers_reject_noninteger_orders(order) -> None:
    with pytest.raises(ValueError, match="order must be an integer"):
        healpix_nested_centers_authalic(order)
    with pytest.raises(ValueError, match="order must be an integer"):
        healpix_nested_cell_indices(order, 0.0, 0.0)
    with pytest.raises(ValueError, match="order must be an integer"):
        wgs84_points_to_healpix_nested(order, 0.0, 0.0)


@pytest.mark.parametrize("order", [0, 4, 9])
def test_query_boundary_rejects_unqualified_healpix_orders(order) -> None:
    source = _synthetic_global_result()
    source = replace(source, config=SimpleNamespace(order=order))
    with pytest.raises(ValueError, match="qualified HEALPix orders"):
