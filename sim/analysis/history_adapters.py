...
def history_from_review_store(path: str | Path, *, object_id: str) -> AnalysisHistory:
    ...
    with sqlite3.connect(database) as conn:
        ...
        time_rows = conn.execute("SELECT sample_index, time_s FROM time_samples ORDER BY sample_index").fetchall()
        rows = conn.execute(
    ...
    expected_indices = list(range(len(time_rows)))
    if [int(row[0]) for row in time_rows] != expected_indices:
        raise ValueError("Review-store time_samples history is incomplete or non-contiguous.")
    if [int(row[0]) for row in rows] != expected_indices:
        raise ValueError(f"Review-store object_state history for {object_id!r} is incomplete or non-contiguous.")
    if not np.allclose(
        np.asarray([row[1] for row in rows], dtype=float),
        np.asarray([row[1] for row in time_rows], dtype=float),
        rtol=0.0,
        atol=1.0e-12,
    ):
        raise ValueError(f"Review-store object_state times for {object_id!r} do not match time_samples.")
    ...
    return history_from_single_run(
        object_id=object_id, times_s=np.asarray([row[1] for row in rows]),
        truth_state=np.asarray([row[2:] for row in rows]),
...
def history_from_ogp_product(product: Any) -> AnalysisHistory:
    ...
    sample_indices = [int(value(row, "sample_index")) for row in samples]
    if sample_indices != list(range(len(samples))):
        raise ValueError("OGP propagation product sample indices must be contiguous and zero-based.")
    times_s = np.asarray([value(row, "t_s") for row in samples], dtype=float)
    jd_utc = np.asarray([value(row, "jd_utc") for row in samples], dtype=float)
    expected_jd_utc = float(product.start_jd_utc) + times_s / 86400.0
    if np.any(~np.isfinite(jd_utc)) or not np.allclose(jd_utc, expected_jd_utc, rtol=0.0, atol=5.0e-12):
        raise ValueError("OGP propagation product sample epochs do not match start_jd_utc plus t_s.")
    declared_count = int(getattr(product, "sample_count", len(samples)))
    if declared_count != len(samples):
        raise ValueError("OGP propagation product sample_count does not match retained samples.")
    return AnalysisHistory(
...
def evaluate_history_global_coverage(
    ...
    *,
    ...
    _require_frame_epoch(history, frame_context, role="coverage source")
...
def evaluate_history_directed_link(
    ...
    *,
    ...
    _require_frame_epoch(tx_history, frame_context, role="directed-link transmitter")
    _require_frame_epoch(rx_history, frame_context, role="directed-link receiver")
    tx_directional = not config.tx_terminal.pattern.attitude_independent
    rx_directional = not config.rx_terminal.pattern.attitude_independent
...
def _require_frame_epoch(history: AnalysisHistory, frame_context: FrameContext, *, role: str) -> None:
    frame_epoch = frame_context.jd_utc_start
    if frame_epoch is None or not np.isclose(
        float(history.initial_jd_utc), float(frame_epoch), rtol=0.0, atol=1.0e-12
    ):
        raise ValueError(
            f"{role} history initial_jd_utc must match the frame context jd_utc_start."
        )
