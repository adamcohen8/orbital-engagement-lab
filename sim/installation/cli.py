...
def _selected_engine(paths: InstallationPaths, workspace: Path | None) -> tuple[str | None, Path, Path, str]:
    if workspace is not None:
        ...
        verified = verify_installation(version, paths=paths, full=False)
    ...
    if current:
        verified = verify_installation(str(current), paths=paths, full=False)
        if verified["status"] in {"official", "developer"}:
...
def _engine_lease(paths: InstallationPaths, version: str | None, disposition: str) -> Iterator[None]:
    if not version or disposition == "developer" or not paths.version_root(version).is_dir():
        yield
        return
