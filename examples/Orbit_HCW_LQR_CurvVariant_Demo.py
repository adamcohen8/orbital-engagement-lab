...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="HCW LQR curvilinear-input variant demo: x_curv -> x_rect (LQR) -> u_rect -> u_eci."
    )
    ...
    ctrl = HCWCurvInputRectOutputController(base_lqr=base)

    x_rel_rect = np.array([6.0, -20.5, 1.5, 0.0008, -0.0004, 0.0003], dtype=float)
    ...
    for k in range(steps):
        x_curv = ric_rect_to_curv(x_rel_rect, r0_km=r0)
        belief = StateBelief(state=np.hstack((x_curv, chief_eci)), covariance=np.eye(12), last_update_t_s=t[k])
        cmd = ctrl.act(belief, t_s=t[k], budget_ms=1.0)
        u_rect = np.array(cmd.mode_flags["accel_rect_ric_km_s2"], dtype=float)
        u_eci = np.array(cmd.mode_flags["accel_eci_km_s2"], dtype=float)
        x_rel_rect = _rk4_step(x_rel_rect, dt_s, n_rad_s, u_rect)
        pos_norm[k + 1] = np.linalg.norm(x_rel_rect[:3])
        vel_norm[k + 1] = np.linalg.norm(x_rel_rect[3:])
        accel_norm[k + 1] = np.linalg.norm(u_eci)

    fig, axes = plt.subplots(3, 1, figsize=(10, 9), sharex=True)
    axes[0].plot(t, pos_norm)
    axes[0].set_ylabel("||r_rel|| (km)")
    axes[0].set_title("Curv-Input Variant: Relative Position Convergence")
    axes[0].grid(True, alpha=0.3)
    axes[1].plot(t, vel_norm)
    axes[1].set_ylabel("||v_rel|| (km/s)")
    axes[1].set_title("Relative Velocity Convergence")
    axes[1].grid(True, alpha=0.3)
    axes[2].plot(t, accel_norm)
    axes[2].axhline(max_accel_km_s2, linestyle="--")
    axes[2].set_ylabel("||a_cmd|| (km/s^2)")
    axes[2].set_xlabel("Time (s)")
    axes[2].set_title("Thrust Command Norm (ECI)")
    axes[2].grid(True, alpha=0.3)
    fig.tight_layout()

    if args.plot_mode in ("save", "both"):
        outdir = REPO_ROOT / "outputs" / "orbit_hcw_lqr_curv_variant_demo"
    ...
    print("HCW Curv-Input Variant Demo complete")
