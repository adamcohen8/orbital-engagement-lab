...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Relative orbit nonlinear MPC demo (curvilinear RIC input -> ECI thrust output, 2-body prediction)."
    )
    ...
    parser.add_argument("--budget-ms", type=float, default=2000.0, help="Controller compute budget per step in milliseconds.")
    ...
    # axes[0].plot(t, pos_norm_passive, "--", label="passive")
    axes[0].set_ylabel("||r_rel|| (km)")
    axes[0].set_title("Relative Orbit MPC: Relative Position Norm")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend(loc="best")

    axes[1].plot(t, vel_norm, label="controlled")
    # axes[1].plot(t, vel_norm_passive, "--", label="passive")
    ...
    for i in range(3):
        ...
        # axes_rel[i, 0].plot(t, rel_passive_hist[:, i], "--", label="passive")
        ...
        # axes_rel[i, 1].plot(t, rel_passive_hist[:, i + 3], "--", label="passive")
    ...
    # axes_proj[0].plot(rel_passive_hist[:, 1], rel_passive_hist[:, 0], "--", label="passive")
    ...
    # Cross-track x, Radial y
    axes_proj[1].plot(rel_hist[:, 2], rel_hist[:, 0], label="controlled")
    # axes_proj[1].plot(rel_passive_hist[:, 2], rel_passive_hist[:, 0], "--", label="passive")
    ...
    # In-track x, Cross-track y
    axes_proj[2].plot(rel_hist[:, 1], rel_hist[:, 2], label="controlled")
    # axes_proj[2].plot(rel_passive_hist[:, 1], rel_passive_hist[:, 2], "--", label="passive")
    ...
    if args.plot_mode in ("save", "both"):
        outdir = REPO_ROOT / "outputs" / "orbit_relative_mpc_demo"
    ...
    print("Relative Orbit MPC Demo complete")
