...
def run_demo(
    ...
    kp_vec = np.array([0.35, 0.35, 0.35], dtype=float) if kp is None else np.array(kp, dtype=float).reshape(3)
    kd_vec = np.array([2.5, 2.5, 2.5], dtype=float) if kd is None else np.array(kd, dtype=float).reshape(3)
    ki_vec = np.array([0.01, 0.01, 0.01], dtype=float) if ki is None else np.array(ki, dtype=float).reshape(3)

    sat = build_sim_object_from_presets(
        object_id="sat_pid_ric_demo",
        dt_s=dt_s,
    ...
    outdir = REPO_ROOT / "outputs" / "attitude_pid_ric_demo"
    ...
    axes_q[0].set_title("RIC-Frame Reaction-Wheel PID: Quaternion")
    axes_q[-1].set_xlabel("Time (s)")
    fig_q.tight_layout()
    q_path = outdir / "pid_ric_quaternion.png"
    if plot_mode in ("save", "both"):
    ...
    axes_w[0].set_title("RIC-Frame Reaction-Wheel PID: Body Rates")
    axes_w[-1].set_xlabel("Time (s)")
    fig_w.tight_layout()
    w_path = outdir / "pid_ric_rates.png"
    if plot_mode in ("save", "both"):
    ...
    fig_t.tight_layout()
    tau_path = outdir / "pid_ric_torque_and_error.png"
    if plot_mode in ("save", "both"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standalone test demo for 3-axis RIC-frame reaction-wheel PID attitude controller.")
    ...
    parser.add_argument("--kp", type=float, default=0.35, help="Scalar Kp for all 3 PID axes.")
    parser.add_argument("--kd", type=float, default=2.5, help="Scalar Kd for all 3 PID axes.")
    parser.add_argument("--ki", type=float, default=0.01, help="Scalar Ki for all 3 PID axes.")
