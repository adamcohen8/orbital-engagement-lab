...
def run_demo(
    ...
    init_yaw_deg: float = 20.0,
    init_pitch_deg: float = -15.0,
    init_roll_deg: float = 25.0,
    omega_x_rad_s: float = 0.008,
    omega_y_rad_s: float = -0.006,
    omega_z_rad_s: float = 0.005,
    kp: np.ndarray | None = None,
    kd: np.ndarray | None = None,
    wheel_scale: float = 100.0,
    frame_skip: int = 1,
    ...
    kd_vec = np.array([10.0, 10.0, 10.0], dtype=float) if kd is None else np.array(kd, dtype=float).reshape(3)

    q_init_bn = dcm_to_quaternion_bn(_dcm_bn_from_euler_zyx_deg(init_yaw_deg, init_pitch_deg, init_roll_deg))
    w0 = np.array([float(omega_x_rad_s), float(omega_y_rad_s), float(omega_z_rad_s)], dtype=float)

    sat = build_sim_object_from_presets(
        object_id="sat_rect_anim_pd_stabilize_eci",
        dt_s=dt_s,
        satellite=BASIC_SATELLITE,
        enable_disturbances=False,
        enable_attitude_knowledge=True,
        attitude_quat_bn=q_init_bn,
    ...
    # Stabilize by holding initial inertial orientation while damping body rates to zero.
    ctrl = ReactionWheelPDController(
        wheel_axes_body=wheel_axes,
        wheel_torque_limits_nm=wheel_limits,
        desired_attitude_quat_bn=q_init_bn,
    ...
    err_hist[0] = _quat_error_deg(q_init_bn, sat.truth.attitude_quat_bn)
    ...
    for k in range(n):
        ...
        err_hist[k + 1] = _quat_error_deg(q_init_bn, sat.truth.attitude_quat_bn)
    ...
    ax.set_title("ECI PD Stabilization: Tumbling Satellite Despin")
    ...
    def _update(i: int):
        ...
        rate_mag = float(np.linalg.norm(w_hist[k, :]))
        txt.set_text(f"t = {t_hist[k]:.1f} s | |w| = {rate_mag:.4f} rad/s")
        return (poly, txt)
    ...
    outdir = REPO_ROOT / "outputs" / "attitude_rectangle_animation_eci_pd_stabilize"
    outpath = outdir / "attitude_rectangle_eci_pd_stabilize.gif"
    ...
    return {
        ...
        "initial_rate_body_rad_s": str(w0.tolist()),
        "final_rate_body_rad_s": str(np.array(w_hist[-1, :], dtype=float).tolist()),
        "final_attitude_hold_error_deg": f"{float(err_hist[-1]):.6f}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Animate ECI PD stabilization from initial tumble (no disturbances)."
    )
    ...
    parser.add_argument("--init-yaw-deg", type=float, default=20.0)
    parser.add_argument("--init-pitch-deg", type=float, default=-15.0)
    parser.add_argument("--init-roll-deg", type=float, default=25.0)
    parser.add_argument("--omega-x", type=float, default=0.08, help="Initial body rate wx (rad/s)")
    parser.add_argument("--omega-y", type=float, default=-0.06, help="Initial body rate wy (rad/s)")
    parser.add_argument("--omega-z", type=float, default=0.05, help="Initial body rate wz (rad/s)")
    ...
    out = run_demo(
        ...
        omega_x_rad_s=float(args.omega_x),
        omega_y_rad_s=float(args.omega_y),
        omega_z_rad_s=float(args.omega_z),
    ...
    print("ECI PD stabilization animation demo outputs:")
