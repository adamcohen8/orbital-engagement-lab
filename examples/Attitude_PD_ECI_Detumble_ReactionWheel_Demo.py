...
from sim.control.attitude import ECIDetumblePDController, ReactionWheelPDController
...
def run_demo(
    ...
    c_bn0 = _rot_z(np.deg2rad(25.0)) @ _rot_y(np.deg2rad(-18.0)) @ _rot_x(np.deg2rad(30.0))
    q0 = dcm_to_quaternion_bn(c_bn0)
    w0 = np.array([0.08, -0.06, 0.05], dtype=float) if w0_body_rad_s is None else np.array(w0_body_rad_s, dtype=float).reshape(3)
    ...
    sat = build_sim_object_from_presets(
        object_id="sat_pd_eci_detumble_demo",
        dt_s=dt_s,
    ...
    ctrl = ECIDetumblePDController(pd=pd, rate_only=True, lock_reference_on_first_call=True)
    sat.controller = ctrl
    ...
    outdir = REPO_ROOT / "outputs" / "attitude_pd_eci_detumble_demo"
    ...
    axes_q[0].set_title("ECI Detumble PD: Quaternion")
    axes_q[-1].set_xlabel("Time (s)")
    fig_q.tight_layout()
    q_path = outdir / "eci_detumble_quaternion.png"
    if plot_mode in ("save", "both"):
    ...
    fig_w.tight_layout()
    w_path = outdir / "eci_detumble_rates.png"
    if plot_mode in ("save", "both"):
    ...
    fig_t.tight_layout()
    tau_path = outdir / "eci_detumble_torque_and_error.png"
    if plot_mode in ("save", "both"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standalone test demo for ECI detumble PD attitude controller with reaction wheels.")
    ...
    parser.add_argument("--w0-x", type=float, default=0.08, help="Initial body rate wx (rad/s).")
    parser.add_argument("--w0-y", type=float, default=-0.06, help="Initial body rate wy (rad/s).")
    parser.add_argument("--w0-z", type=float, default=0.05, help="Initial body rate wz (rad/s).")
    ...
    print("ECI detumble PD demo outputs:")
