...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Predictive EKF coupling demo with reaction-wheel PD attitude control."
    )
    parser.add_argument("--plot-mode", choices=["interactive", "save", "both"], default="interactive")
    parser.add_argument("--dt", type=float, default=1.0)
    parser.add_argument("--orbit-dt", type=float, default=1.0, help="Optional orbital integration substep (s).")
    parser.add_argument("--attitude-dt", type=float, default=0.01, help="Optional attitude integration substep (s).")
    parser.add_argument("--duration", type=float, default=10000.0)
    parser.add_argument("--lead-time-s", type=float, default=20.0, help="Lead time (seconds) between attitude command and burn check.")
    ...
    x_rel0_rect = np.array([0.0, -8.0, 0.0, 0.000, 0.000, 0.000], dtype=float)
    ...
    att_pd = ReactionWheelPDController(
    ...
    for k in range(steps):
        ...
        q_target = np.array(decision["planned_q_target_bn"], dtype=float)
        att_pd.set_target(q_target, np.zeros(3))
        ...
        c_att = att_pd.act(belief_att, t_s=chaser.truth.t_s, budget_ms=1.0)
        thrust = np.zeros(3)
        if decision["fire"]:
    ...
    axes[0].set_title("Predictive EKF Coupling (RW-PD): Relative Position")
    ...
    if args.plot_mode in ("save", "both"):
        outdir = REPO_ROOT / "outputs" / "rendezvous_predictive_ekf_demo_rw_pd"
    ...
    print("Attitude controller:", "reaction_wheel_pd")
